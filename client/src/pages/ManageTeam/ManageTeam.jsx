...
const ManageTeam = () => {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const res = await resetUserPassword(resetUser._id, newTempPassword);
    ...
    <div className="manage-team">
      <div className="team-header-card glass-panel">
        <div className="header-info">
          ...
          <p className="subtitle">Manage member usernames and reset passwords if a freelancer forgets theirs</p>
        </div>
      ...
      <div className="team-members-grid">
        {team.length === 0 ? (
          <div className="empty-state glass-panel">
            ...
            <p>Click "Add New Member" to add freelancers to your workspace.</p>
          </div>
        ) : (
          team.map((member) => (
            <div key={member._id} className="member-card glass-panel">
              <div className="member-card-header">
                <div 
                  ...
                  style={{ backgroundColor: member.avatar || '#6C5CE7' }}
                >
                  {member.name ? member.name.charAt(0).toUpperCase() : 'U'}
                </div>
                <span className={`role-badge role-${member.role}`}>
                  {member.role === 'admin' ? <FiShield size={12} /> : <FiUser size={12} />}
                  {member.role}
                </span>
              </div>
              ...
              <div className="member-card-footer">
                <button
                  className="btn-action-member btn-reset-password"
                  onClick={() => {
                    setResetUser(member);
                    setNewTempPassword('');
                    setResetError('');
                ...
                <button 
                  className="btn-action-member btn-delete-member" 
